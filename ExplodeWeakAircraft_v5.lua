...
local function explodeUnit(_unit, _typeName, _healthTrigger, _explosionPower, _flares)
	...
	if health <= _healthTrigger and health > .05 then -- If health is below the threshold from the USER SETTINGS ,trigger explosion on the unit until health falls below 5%.
        if _flares then
